...
* Innter Functions
*****************************************************************************/
function __get_record__(data, ipnum) {
...
// synchronous method
var record_by_addr = function(data, addr) {
  ...
  ipnum = ip2long(addr);
  record = __get_record__(data, ipnum);
  if (!record) {
    return null;
  } else {
    return record;
...
record_by_addr._help = 'Get city record by ip addClass.';

// Asynchronous method
var record_by_domain = function(data, domain, callback) {
  ...
  if (data.db_type !== CONST.CITY_EDITION_REV0 &&
    ...
    dns.resolve4(domain, function(error, addresses) {
      ...
      seek = __get_record__(data, ipnum);
      if (!seek) {
        record = null;
...
record_by_domain._help = 'Get city record by domain.';

exports.record_by_addr = record_by_addr;
exports.record_by_domain = record_by_domain;
