...
var net = require('net'),
    ...
    addr_to_ipnum = require('../build/default/ipnum.node').addr_to_ipnum;

/*****************************************************************************
* Inter Functions
****************************************************************************/
// Validate IPv4 address
function __isIPv4__(addr) {
...
var ipToLong = function(addr) {
  ...
  if (typeof net.isIPv4 === 'function') {
    isValid = net.isIPv4(addr);
  } else {
    isValid = __isIPv4__(addr);
  }

  if (!isValid) {
    return false;
  } else {
    long = addr_to_ipnum(addr);
  }
