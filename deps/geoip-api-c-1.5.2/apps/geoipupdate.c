...
void print_status(int err, char *license_file)
{
    if (err == GEOIP_NO_NEW_UPDATES) {
        ...
        fprintf(stderr,
                "Invalid License Key in %s - Please visit http://www.maxmind.com/app/products for a subscription\n",
                license_file);
        ...
        fprintf(stderr,
                "Received Error %d (%s) when attempting to update GeoIP Database\n",
...
int main(int argc, char *argv[])
{
    ...
    if (license_file == NULL) {
        license_file =
            malloc(sizeof(char) *
                   (strlen(SYSCONFDIR) + strlen(GeoIPConfFile) + 2));
        license_file[0] = '\0';
    ...
    do {
        ...
        if (c == 10 || c == EOF) {
            ...
            if (a_product_id_str != NULL) {
                ...
                while (a_ptr[0] != '\0') {
                    ...
                    while ((a_ptr[0] != ' ') & (a_ptr[0] != '\0')) {
                        if (the_product_id_strl[num_product_ids] >=
                        ...
                        the_product_id_str[num_product_ids][the_product_id_strl
                                                            [num_product_ids]] =
                            a_ptr[0];
                    ...
                    /* array of string */
                    the_product_id_str =
                        (char **)realloc_or_die(the_product_id_str,
                                                (num_product_ids +
                                                 1) * sizeof(char *));
                    /* array of string lengths */
                    the_product_id_strl =
                        (int *)realloc_or_die(the_product_id_strl,
                                              (num_product_ids +
                                               1) * sizeof(char *));
                    /* array of string alloc lengths */
                    the_product_id_stral =
                        (int *)realloc_or_die(the_product_id_stral,
                                              (num_product_ids +
                                               1) * sizeof(char *));
                }
            }
            ...
            if (a_user_id_str != NULL) {
                ...
                while ((a_ptr[0] >= '0') & (a_ptr[0] <= '9')) {
                    ...
                    if (the_user_id_strl >= the_user_id_stral) {
                        ...
                        the_user_id_stral += 20;
                        the_user_id_str =
                            realloc_or_die(the_user_id_str, the_user_id_stral);
                    }
                }
            ...
            if (a_license_key_str != NULL) {
                ...
                the_license_key_str =
                    malloc(sizeof(char) * (LICENSE_KEY_LENGTH + 1));
    ...
    if (the_user_id_str != NULL) {
        ...
        for (i = 0; i < num_product_ids; i++) {
            err =
                GeoIP_update_database_general(the_user_id_str,
                                              the_license_key_str,
                                              the_product_id_str[i], verbose,
                                              &client_ipaddr, &my_printf);
